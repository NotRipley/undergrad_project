...
simple_data <- function(m, n, N, k, overlap, equalin) {
  ...
  merged <- do.call(rbind, unmerged) # smush the matrices together
  
  # at this point, merged is just a matrix. We want to add N columns of normally-distributed noise. 
  
  noise <- matrix(rnorm(N*m*k, mean = 0, sd = 0.3), nrow = m*k, ncol = N) # we choose a sd for these noise dimensions that isn't bigger or smaller than any of the clusters would be, because I think that could create more problems. 
  
  merged_2 <- cbind(merged, noise) # chuck em together and return result
  
  return(as.data.frame(merged_2))
...
test <- simple_data(50, 5, 0, 5, FALSE, TRUE)
