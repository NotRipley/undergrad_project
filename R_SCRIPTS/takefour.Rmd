...
Denote by the word 'Simple', a completely artificial, arbitrary dataset created by me. Just for the purposes of testing the performance of algorithms/metrics across generation method and across dimension, for example. 

So we have Simple Separated Data, and Simple Overlapping Data. I think then, we also want Equal-In (spherical) and Equal-Out (equal variance between clusters). So that's 6 different types of data.

Then, we want to get Splatter simulated data up to a similar level of understanding as that top thing there. But the fact that we have these 6 means that it's likely we're going to have 


### Types of Index / Metric
...
library(scater) # for the realistic dataset?
library(fpc) # this and dbscan for dbscan clustering.
library(dbscan)
...
# kmeans_true_k will only work on datasets where we have the true number of clusters. we will use the paper that cecilia sent over to do a kmeans_est_k when we have replicated the simple simulation. This is justified by the fact that the simple simulation's aim is not to implicitly assess the performance of a clustering esimator, so we are essentially controlling for this. 
...
test <- simple_data(50, 5, 1, 5, FALSE, TRUE)

kmeans_on_test <- kmeans_true_k(test, 5, TRUE)

kmeans_on_test

ggplot(test, aes(x = test[,1], y = test[,2])) +
  geom_point()
...
```{r Replicating Simple Simulation - Data Generation}

# first, we want to generate our huge dataset. 
# it is a bad idea to do this with no progress print statements. 

sim_data <- simple_data(2000, 10, 10^4, 4, TRUE, TRUE)

```

```{r Replicating Simple Simulation - Running KMeans}

# we want to slice the data and run KMeans as the number of noise dimensions increases. At each step we want to calculate adjusted rand index and add it to a vector, which we will then plot. 

# our data is stored in sim_data

# we make our log sequence like before
log_sequence <- unique(round(10^seq(1, 4, length.out = 30)))

# initialise a vector to store our scores
scores <- numeric(length(log_sequence))

for (i in seq_along(log_sequence)) {
  current_slice <- sim_data[, 1:log_sequence[i]]
  kmobject <- kmeans_true_k(current_slice, 3, FALSE) # we don't ignore last because we never pass it all 10011
...
simple_sample_dbscan <- function(k, n, M, N, v) {
  ...
  for (i in seq_along(v)) {
    ...
    minpts = ncol(current_slice)
    
    eps <- quantile(dbscan::kNNdist(current_slice, k = minpts), 0.90)
...
```{r Trying simple sample dbscan}

log_sequence <- unique(round(10^seq(1, 4, length.out = 30)))

output_db <- simple_sample_dbscan(k = 5, n = 10, M = 200, v = log_sequence, N = max(log_sequence))
...
# what we find is that i think some auto - generation of eps and minpts is essential. need to do some reading on that.

# minpts should be equal or greater than the number of dimensions. as a rule of thumb, go with double. 

# we go with KNN methods for choosing eps. the KNN graph is the cubic-looking thing of KNN distance vs observations
...
Now, we want to format this rmd doc properly so that we can really see what we have. Go back through the KMeans simple sim and streamline it. We want a function that makes a dataset and a function that scores it. Then, we want to make a new section for the splatter data creation. That's pretty much all we have for now. Then, we can start to fill in the gaps, once it's formatted correctly. 
