...
# sobject - the seurat object we will process
# umap_dims - the number of dims to consider in our umap step
# nbr_dims - number of dims considered in our find neighbours step
# reg_vars - assumed that PercentageFeatSet has been run or similar beforehand, and that the cols we want to control for already exist. So we add them here. char or vector of chars
seurat_preprocess <- function(sobject, umap_dims = 30, nbr_dims = 30, reg_vars = NULL) {
  
  stopifnot(
    is.numeric(umap_dims),
    is.numeric(nbr_dims)
  )
  
  # run main SCTransform step
  if (is.null(reg_vars)) {
    sobject <- SCTransform(sobject, verbose = FALSE)
  } else {
    sobject <- SCTransform(sobject, vars.to.regress = reg_vars, verbose = FALSE)
  }
  
  sobject <- RunPCA(sobject, verbose = FALSE)
  sobject <- RunUMAP(sobject, dims = 1:umap_dims, verbose = FALSE)

  sobject <- FindNeighbors(sobject, dims = 1:nbr_dims, verbose = FALSE)
  sobject <- FindClusters(sobject, verbose = FALSE)
  
  return(sobject)
  
}

seurat2 <- seurat_preprocess(seurat2)

DimPlot(seurat2)

# i can't tell why these are incredibly well separated, if it's just because UMAP is cooking or what. That'll be one of the next tasks. 

# but yeah, now we need to extract just how good of a job the algorithms have done here. As standard, the clustering in this workflow is Louvain i think.
...
### Performance on Real Data - pcmb one?

```{r Standard Workflow on pcmb Data}
